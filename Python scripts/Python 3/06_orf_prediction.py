...
parser.add_argument("-o", "--outfolder_invalid_folder", type=str,
...
def translate_orf_positions(orfstart, orfstop, offset, frame, seq):
    ...
    tmp_protein = str(Seq(cdna[offset:]).translate(table=5))
    #print("cDNA sequence:", cdna[offset:])
...
def extend_forward(protein, protstart):
    ...
    for i, p in enumerate(protein[int(protstart):]):
        if p == "*":
            ...
            print(i)
            print(len(protein[int(protstart):])-1)
            print(i+offset)
            return i+offset
...
def main():
            ...
            with open(translated_outfile_path, "wb") as query_outfile, open(cdna_outfile_path, "wb") as cdna_outfile:
                for hit in blastout:
                    ...
                    if extended_protstart != None:
                        ...
                        tmp_protein = str(Seq(extended_cdna).translate(table=5))
                        if tmp_protein != extended_prot:
                            print ('ERROR: Translated cDNA does not match extended Protein!')
                            sys.exit(0)
                        query_outfile.write((">%s %s \n" % (seq, qframe)).encode()) # Sylvie edit: query_outfile.write(">%s %s \n" % (seq, qframe)) !!!!!
                        query_outfile.write((extended_prot + "\n").encode()) # Sylvie edit: query_outfile.write(extended_prot+"\n") !!!!!
                        cdna_outfile.write((">%s %s \n" % (seq, qframe)).encode()) # Sylvie edit: cdna_outfile.write(">%s %s \n" % (seq, qframe)) !!!!!!!
                        cdna_outfile.write((extended_cdna + "\n").encode()) # Sylvie edit: cdna_outfile.write(extended_cdna+"\n")
                    else:
                        print('no')
                        query_outfile.write("Invalid open reading frame".encode()) #Sylvie edit:  query_outfile.write("Invalid open reading frame")
                        cdna_outfile.write("Invalid open reading frame".encode()) #Sylvie edit: cdna_outfile.write("Invalid open reading frame")
    ...
    if not os.path.isdir(args.outfolder_invalid_folder): os.mkdir(args.outfolder_invalid_folder)
    for o in list(set(invalid_orthogroups)):
        import shutil
        shutil.move(o, args.outfolder_invalid_folder)
