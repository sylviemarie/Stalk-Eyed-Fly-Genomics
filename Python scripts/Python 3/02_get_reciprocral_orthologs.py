...
parser.add_argument("-i", "--input", required=True, type=str,
                    help="Path to folder containing blastout files")
parser.add_argument("-o", "--output", required=True, type=str,
                    help="The prefix of the outfile [script adds .pkl and .txt]", default="./output")
...
# ==============================================================================
def main():
    infiles = list_folder(args.input)
    ...
    outfile_name = os.path.join(os.getcwd(), "output.pkl")
    with open(outfile_name, "wb") as outfile:
        pickle.dump(recip_besthit_all, outfile, -1)

    print('Writing to file ...')
    outtext_name = os.path.join(os.getcwd(), "output.txt")
    with open(outtext_name, "w") as outtext:
        for gene in recip_besthit_all:
            ...
            for g in gene:
                ...
                outtext.write(str(g))
                outtext.write("\n")
            # Move print statement to inside the for loop
            print(f'Number of reciprocal tophits = {count}')
